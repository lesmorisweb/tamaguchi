...
import {TicTacMessage} from "../../Components/games/tik-tak-toe/ticTacMessage/TicTacMessage.tsx";
...
export function Screen({petIsAlive, petName, petAge, petFun, petHeart, petHungry, petColor}: IScreen) {
   ...
   function onPlayerWin() {
      <TicTacMessage message={ "Attention" } title={"Good work!! you win..!"}/>;
   ...
   function onPlayerLose() {
      <TicTacMessage message={ "Attention" } title={"Try again, best luck next time.."}/>;
   ...
   function onEven() {
      ...
      happinessModify(-2);
      setCurrentScreen("game");
      <TicTacMessage message={ "Attention" } title={"Too near, try again"}/>;
   }
   ...
   return (
      <div className={ "screen" }>
         ...
         {
            showFeedForm &&
            <Feed onFeedDisplay={ onHeaderFeed } hungryStats={ hungryCharacter } onFeedClick={feed} goBackButton={backToGame}/>
         }
         {
            showPlayTogether &&
            <PlayTogether/>
